pipeline {
  ...
  environment {
    GITNAME = 'JaejinLee1215'
    GITEMAIL = 'leejaejin1215@gmail.com'
    GITWEBADD = 'https://github.com/JaejinLee1215/jenwp.git'
    GITSSHADD = 'git@github.com:JaejinLee1215/jenwp.git'
    GITDEPADD = 'git@github.com:JaejinLee1215/jenwp.git'
  ...
  stages {
    stage('Checkout Github') {
      steps {
	slackSend (channel: '#jenkins_leejaejin', color: '#FFFF00', message: "STARTED: Job '${env.JOB_NAME} [${env.BUILD_NUMBER}]' (${env.BUILD_URL})")
	...
	sh "docker build -t ${DOCKERHUB2}:${currentBuild.number} -f db-Dockerfile ."
        sh "docker build -t ${DOCKERHUB2}:latest -f db-Dcokerfile ."
